...
# Frame da tabela: =================================================================================
frm = LabelFrame(janela, text='Dados do Excel',
                 bg='#f1f8e9', width=700, height=300)
...
rolagem_horizontal = ttk.Scrollbar(
    frm, orient='horizontal', command=tabela.xview)
tabela.configure(xscrollcommand=rolagem_horizontal.set,
...
# Frame dos widget: ================================================================================
frm02 = LabelFrame(janela, text='Ferramentas',
                   bg='#f1f8e9', width=700, height=250)
...
btn2 = Button(frm02, text='Carregar', bg='#5efc82',
              command=lambda: load_arquivo())
btn2.place(x=150, y=100)

btn_filtro = Button(frm02, text='Filtrar', bg='#5efc82',
                    command=lambda: filter_data())
btn_filtro.place(x=280, y=100)


def folder():
    arquivo_excel = filedialog.askopenfilename(title='Selecione o arquivo:', filetypes=(
...
def filter_data():
    ...
    clear_data()
    df = pandas.DataFrame(carregar).drop(columns='#')

    tabela['column'] = list(df.filter(
        items=['Departamento', 'Categoria']))
    tabela['show'] = 'headings'
